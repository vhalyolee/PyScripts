    ...
    with open(filename) as star_file:
        for line in star_file:
            ...
            stars.append({
                ...
                'apparent_magnitude': float(line[29:34]),
                'absolute_magnitude': float(line[35:40]),
                'parallax': float(line[41:46])
            })
    return stars
...
def write_stars(filename, data, comment=None):
    ...
    with open(filename, 'w') as star_file:
        ...
        # write data
        star_format = "{name:18s}{spectral_class:10s}{apparent_magnitude:>6.2f}" +\
            "{absolute_magnitude:>6.2f} {parallax:0=5.1f}\n"
        for star in data:
            star_file.write(star_format.format(**star))
